...
kirchner.cv counts its own page views (owner 2026-09-16: analytics for the sites,
...
few Netlify credits a month. The same code runs on kirchner.ink; lawsofexistence.com
carries its port.
...
| the visitor's switch | `app/_components/AnalyticsOptOut.tsx` on `/privacy` |
...
turned counting off on the Privacy page (`localStorage['jk-analytics'] = 'off'`),
