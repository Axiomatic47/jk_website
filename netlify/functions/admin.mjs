...
function signInPage(cfg, url) {
  ...
  const body = `<h1>Kirchner.cv — console</h1><p class="sub">Sign in to moderate Open Readings answers.</p>
...
async function home(url, sess, csrf) {
  ...
  const body = `<h1>Console</h1><p class="sub">Signed in as ${h(sess.email)}. Sessions last eight hours. Queue store: <code>${h(c.kind)}</code>${c.kind.startsWith('blobs') ? '' : ' <b class="warn">— not Netlify Blobs; submissions cannot be queued</b>'}.</p>
<div class="grid">
